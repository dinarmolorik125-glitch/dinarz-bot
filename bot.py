...
async def remove_bg(callback: CallbackQuery):
    ...
    if not file_id:
        await callback.message.answer("📸 Сначала отправь фотографию.")
        return

    status = await callback.message.answer("⏳ Загружаю фотографию...")

    try:
        telegram_file = await bot.get_file(file_id)

        data = BytesIO()
        await bot.download_file(telegram_file.file_path, data)
